...
exports.updatePayrollRunStatus = exports.generatePayrollRun = exports.getPayrollRun = void 0;
...
const getPayrollRun = async (req, res) => {
    try {
        ...
        const run = await prisma_1.prisma.payrollRun.findUnique({
            where: {
...
const generatePayrollRun = async (req, res) => {
    try {
        ...
        const settings = await prisma_1.prisma.appSettings.findFirst();
        if (!settings) {
        ...
        const attendanceList = await prisma_1.prisma.employeeMonthlyAttendance.findMany({
            where: { month, year },
            select: { employeeId: true, presentDays: true, otHours: true },
        });
        const attendanceMap = new Map(attendanceList.map((a) => [
            a.employeeId,
            {
        ...
        const run = await prisma_1.prisma.payrollRun.upsert({
            where: {
                month_year: { month, year },
            },
            update: {},
        ...
        await prisma_1.prisma.payrollItem.deleteMany({
            where: { payrollRunId: run.id },
        });
        // ✅ Create fresh payroll items (snapshot)
        const itemsToCreate = employees.map((emp) => {
            const monthlySalary = Number(emp.salary || 0);
            const attendance = attendanceMap.get(emp.id) || { presentDays: 0, otHours: 0 };
            ...
            if (settings.otType === "FIXED") {
                ...
                // MULTIPLIER type
                const hourlyPay = workingDays > 0 && workingHours > 0
        ...
        await prisma_1.prisma.payrollItem.createMany({
            data: itemsToCreate,
        });
        const finalRun = await prisma_1.prisma.payrollRun.findUnique({
            where: { id: run.id },
            include: {
                items: {
                    include: {
...
exports.generatePayrollRun = generatePayrollRun;
const updatePayrollRunStatus = async (req, res) => {
    try {
        ...
        const run = await prisma_1.prisma.payrollRun.findUnique({ where: { id } });
        if (!run) {
        ...
        const updated = await prisma_1.prisma.payrollRun.update({
            where: { id },
            data: { status },
        });
        return res.status(200).json({
...
exports.updatePayrollRunStatus = updatePayrollRunStatus;
